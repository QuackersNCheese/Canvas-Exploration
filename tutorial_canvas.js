...
let xmid = canvas1.width / 2;
let ymid = canvas1.height / 2;
...
function plotParametric(v, r, resolution = 100) {
  ...
  for(let t = r[0]; t <= r[1]; t += (r[1] - r[0]) / resolution) {
    ...
    context.fillStyle = `hsla(${t * 90 % 360}, 100%, 50%, 1)`;
    context.fillRect(xmid + x_func(t), ymid - y_func(t), 2, 2);
  }
}
...
async function petals() {
  while(true) {  
    for(let n = 0 ; n < 360; n += 0.001) {
      plotParametric([t=>xmid * Math.cos(n*t) * Math.cos(t), 
                      t=>ymid * Math.cos(n*t) * Math.sin(t)], 
                      [-2 * Math.PI, 2 * Math.PI], 
                      1000);
...
window.addEventListener("load", petals, false);
...
window.addEventListener("load", paraboloid, false);